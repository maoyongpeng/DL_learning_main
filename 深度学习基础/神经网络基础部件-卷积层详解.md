...
根据卷积层的定义，卷积层有两个很重要的性质: 局部连接和权重共享。此外还有汇聚(池化的意思)。
...
1. 在来自卷积核的元素和输入特征图子矩阵的元素之间进行乘法以获得输出感受野(特定感受刺激才能激活神经元的区域)。 
...
注意，卷积层的输出 `Feature map` 的大小(W,W)取决于输入的大小、Pad 数、卷积核大小和步长。在 `Pytorch` 框架中，图片（`feature map`）经卷积 `Conv2D` 后**输出大小计算公式**如下：
$$
...
上图是三通道经过两组过滤器的卷积过程，在这个例子中，输入是三维数据 $
$$
3\times 32 \times32
$$
$，经过权重参数尺寸为 
...
2\times28\times28
$$
$，维数并没有变化，只是每一维内部的尺寸有了变化，一般都是要向更小的尺寸变化，以便于简化计算(积的意义所在)。

假设三维卷积核（也叫滤波器）尺寸为 
$$
(c_{in}, k, k)
$$
，一共有 $c_{out}$ 个滤波器，即卷积层参数尺寸为 
$$
(c_{out}, c_{in}, k, k)
$$
 ，则标准卷积层计算有以下**特点**：
...
5. 卷积核 Kernal 的大小一般是奇数如：$1\times 1$，$3\times 3$。当然也可以为偶数。
...
class torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=Truew)
```
...
在这两种情况下，与互相关运算符一样，汇聚窗口从输入张量的左上⻆开始，从左往右、从上往下的在输入张量内滑动。在汇聚窗口到达的每个位置，它计算该窗口（注意是这个窗口内部）中输入子张量的最大值或平均值。计算最大值或平均值是取决于使用了最大汇聚层还是平均汇聚层。

比如最大汇聚层，其计算输入中区域的最大值。

![最大汇聚层计算结果](../images/conv/pool_cal2.png)

值得注意的是，与卷积层一样，汇聚层也可以通过改变和步幅以获得所需的输出形状。
$$
N=\left \lfloor \frac{W-K}{S} \right\rfloor+1 
\\W代表输入特征图的尺寸（正方形）
\\K为卷积核的大小
\\S为步长，通常不进行填充。
$$
